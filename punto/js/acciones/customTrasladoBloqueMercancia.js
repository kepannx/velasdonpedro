...
document.getElementById('nombreProducto').addEventListener('change', function(){
      ...
      if ((skuserializado).length > 0) {
          $.ajax({
            ...
            type    : 'POST',
            data: { sku: ""+skuserializado+"" },
            success :  function(data){
               if (data=='true') { //Es sin seriales
                loadFeaturesFromSku(skuserializado); 
               }
               else if(data=='false'){
    ...
    if (sku.length > 0) {
      ...
      $.ajax({
          ...
          success: function (data) {
            ...
            var promesa = checkexistencia(origenId.value, datos.idproductosServicios);
